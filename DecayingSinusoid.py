...
from Bioluminescence import Bioluminescence
...
class SingleModel(object):
    ...
    def create_parameters(self, master):
        ...
        # Note that period must be a minimum of 1/nyquist_freq
        params.add('amplitude', value=master.p0['amplitude'], min=0)
        params.add('period', value=master.p0['period'],
                   min=2*np.diff(self.x).min(),
                   max=(self.x.max() - self.x.min()))
    ...
    def fit(self):
        """ Fit the function to the data. Note that if the function returns
        a NaN it will return a ValueError. """

        # In cases where NaNs arise, this will return an error
        ...
        # Fitted values
        self.sinusoid = sinusoid_component(self.result.params, self.x)
        self.baseline = baseline_component(self.result.params, self.x)
...
#This does not work for my data from Trackmate, try to change parameters
...
class DecayingSinusoid(object):
    ...
    def run(self):
        ...
        self._fit_models(period_force=None)
    ...
    def _fit_models(self, period_force=None):
        # adding some error checking
        # period force will bound the period within 1h of the value
        self.models = []
        start = self.max_degree if self.opt['specific_degree'] else 0
        for i in range(start, self.max_degree+1):
            self.models += [SingleModel(self.x, self.y, i)]
            self.models[-1].create_parameters(self)
            # if period_bound, replace value
            if period_force != None:
                self.models[-1].params.add('period',
                        value=period_force, min=period_force-1,
                        max=period_force+1)
            try:
                self.models[-1].fit()
            except ValueError:
                # if the optimization can't fit the model, delete it
    ...
    def _hilbert_fit(self):
        ...
        Bioluminescence module for a second opinion. (Deprecated) """
...
class ModelAveragedParameter(object):
    def __init__(self, key, models, weights):
        ...
        means = np.array([model.result.params[key].value for model in
                          self.models])
        variances = []
        variances_defined = True
        for model in self.models:
            try:
                variances.append(model.result.params[key].stderr**2)
            except TypeError:
                # undefined variances
                variances.append(0)
                variances_defined = False

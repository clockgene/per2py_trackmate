...
def estimate_period(x, y, period_low=1, period_high=100, res=200):
    ...
    periods, pgram, _ = periodogram(x, y, period_low=period_low,
...
def fit_decaying_sinusoid(x, y, weights=None, bounds=None):
    """ Estimate and fit parameters for a decaying sinusoid to fit y(x)
    """

    p0 = _pars_to_plist(estimate_sinusoid_pars(x, y))
    lsp_period = estimate_period(x,y)
    if weights is None: weights = 1/np.exp(-p0[-1]*x)
    #if weights is 'capped': weights = 0.1 + 1/np.exp(-p0[-1]*x)
    if weights == 'capped': weights = 0.1 + 1/np.exp(-p0[-1]*x)
    if bounds is None: bounds = (-np.inf,np.inf)
    #if bounds is 'period':
    if bounds == 'period':
        # if we are getting the fit from the periodogram
        bounds=([-np.inf,lsp_period-2,-np.inf,-np.inf],
                [np.inf ,lsp_period+2, np.inf, np.inf])
        p0[1] = lsp_period
...
def calculate_widths(x, shortestperiod=20., longestperiod=30.,
    ...
    pow_arr = np.hstack([np.arange(nvoice) for i in range(noctave)])+1
...
def timeseries_boundary(x, opt_b, bdetrend):
    ...
    if opt_b != 'per': index = np.arange(M/2, M + M/2)
...
def timeseries_boundary_old(x, opt_b, bdetrend):
    ...
    if opt_b != 'per': index = np.arange(M, 2*M)
    else: index = np.arange(M)
...
def dwt_breakdown(x, y, wavelet='dmey', nbins=np.inf, mode='sym'):
    ...
    dx = x[1] - x[0]
    period_bins = [(2**j*dx, 2**(j+1)*dx) for j in range(1,nbins+1)]
